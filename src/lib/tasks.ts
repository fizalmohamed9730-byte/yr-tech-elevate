...
export interface TaskDef {
  ...
  title: string;
  description: string;
  requires: { github?: boolean; project?: boolean; drive?: boolean };
}
...
const task = (
  ...
  title: string,
  description: string,
  requires: { github?: boolean; project?: boolean; drive?: boolean } = { github: true, project: true, drive: true },
): TaskDef => ({ no, title, description, requires });

export const DOMAIN_TASKS: Record<DomainSlug, TaskDef[]> = {
  "full-stack": [
    task(1, "Responsive Multi-Page Landing Website", "Build a responsive, multi-section landing website using HTML, CSS, and JavaScript (or a framework). The site must include a navbar, hero section, features section, testimonials, and a contact form with validation. Deliverables: GitHub repository, live deployment URL, and a brief README explaining your approach."),
  ...
  "ui-ux": [
    task(1, "UX Research & Problem Definition", "Conduct UX research for a mobile or web application. Define the problem statement, identify target users, create 2-3 user personas, conduct competitive analysis of 3 existing products, and document user pain points. Deliverables: Research document covering problem definition, personas, and competitive analysis."),
    task(2, "User Flows & Information Architecture", "Design complete user flows and information architecture for the application from Task 1. Create task flows for core user journeys, define the sitemap and navigation structure, and map out the screen hierarchy. Deliverables: User flow diagrams, sitemap, and IA documentation."),
    task(3, "Wireframes & Low-Fidelity Prototypes", "Create wireframes for all core screens (minimum 8 screens). Build low-fidelity clickable prototypes in Figma demonstrating the primary user journey. Include annotations explaining interaction patterns and layout decisions. Deliverables: Figma wireframe file with clickable prototype and annotation notes."),
    task(4, "High-Fidelity Design System & Screens", "Design high-fidelity screens for all core screens using a consistent design system. Define typography, color palette, spacing system, component library (buttons, inputs, cards, navigation), and iconography. Ensure responsive designs for mobile and desktop. Deliverables: Figma file with design system and all high-fidelity screens."),
    task(5, "Interactive Prototype & Usability Report", "Create a fully interactive high-fidelity prototype in Figma with realistic interactions, transitions, and micro-animations. Conduct a heuristic evaluation using Nielsen's heuristics. Document usability considerations, accessibility notes (WCAG), and design handoff specifications. Deliverables: Interactive Figma prototype, heuristic evaluation report, and handoff documentation."),
  ],
  python: [
