...
import { z } from "zod";
import { getEmailConfig } from "@/lib/config.server";
import { generateOfferLetterPDFBuffer, generateCertificatePDFBuffer } from "@/lib/pdf.server";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import { Resend } from "resend";

const OFFER_LETTER_HTML = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  ...
  <div style="max-width:600px;margin:0 auto;padding:32px 24px;">
    ...
    <div style="background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;padding:32px;">
      ...
      <div style="background:#f5f7fa;border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin:20px 0;">
        <p style="margin:0 0 8px;font-size:14px;"><strong>Internship ID:</strong> {{internshipCode}}</p>
        <p style="margin:0 0 8px;font-size:14px;"><strong>Domain:</strong> {{domain}}</p>
        <p style="margin:0;font-size:14px;"><strong>Duration:</strong> {{duration}}</p>
      </div>
    ...
    try {
      ...
      try {
        ...
        const response = await resend.emails.send({
          ...
          html: fillTemplate(OFFER_LETTER_HTML, {
            fullName: data.fullName,
            internshipCode: data.internshipCode,
            domain: data.domain,
            duration: data.duration,
      ...
      console.error(`[email] UNHANDLED ERROR in sendOfferLetterEmail:`, err);
    ...
    try {
      ...
      try {
        ...
        const response = await resend.emails.send({
          ...
          html: fillTemplate(CERTIFICATE_HTML, {
            fullName: data.fullName,
            internshipCode: data.internshipCode,
            domain: data.domain,
            duration: data.duration,
      ...
      console.error(`[email] UNHANDLED ERROR in sendCertificateEmail:`, err);
      return { success: false, error: `Server error: ${err?.message ?? "Unknown error"}` };
    }
  });
